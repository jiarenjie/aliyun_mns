...
-record(state, {accountId, accessId, accessKey, topic}).
...
handle_call({process, #{templateCode := TemplateCode, msg := Msg, signName := SignName} = _Map}, _From,
    #state{accessId = AccessId, accessKey = AccessKey, accountId = AccountId, topic = Topic} = State) ->
  Uri = get_uri(Topic),
  ...
  [Type, Num, SmsParams] = ali_utils:msg_convent(Msg),
